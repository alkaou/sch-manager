...
export default function EventsPage() {
  ...
  const loadEvents = useCallback(async () => {
    try {
      setIsLoading(true);
      const eventsData = await getAllEvents(database);
  ...
  }, [loadEvents, language, setFlashMessage]);
  ...
  const confirmDelete = async () => {
    if (eventToDelete) {
      try {
        await deleteEvent(eventToDelete.id, database, setFlashMessage, language);
        await refreshEvents();
        ...
        <AnimatePresence>
          ...
          {showDeleteConfirm && eventToDelete && (
            <ActionConfirmePopup
              ...
              message={`${translate('confirm_delete_event', language)} "${eventToDelete.title}" ?`}
              actionType="delete"
              elementInfo={eventToDelete.title}
            />
          )}
        </AnimatePresence>
      </div>
