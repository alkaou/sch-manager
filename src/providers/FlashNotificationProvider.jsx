...
import { FlashNotificationContext, useFlashNotification } from '../components/contexts';
...
const FlashNotificationProvider = ({ children }) => {
  ...
  const setFlashMessage = useCallback(({ message, type = 'info', duration = 3000 }) => {
  ...
  return (
    <div
      className={`fixed bottom-5 right-5 z-50 transition-all duration-700 ease-in-out transform 
        ${flash.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
    >
      <div
