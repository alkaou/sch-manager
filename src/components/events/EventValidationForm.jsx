...
const EventValidationForm = ({
  ...
  const [formData, setFormData] = useState({
    success: event.validation?.success || "",
    failure: event.validation?.failure || "",
    remarks: event.validation?.remarks || "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const validatedEvent = await validateEvent(
        event.id,
        formData,
        database,
        setFlashMessage,
  ...
  const getTextareaClasses = (hasError = false) => {
    const baseClasses =
      "w-full px-4 py-3 rounded-lg border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none";
    const themeClasses =
      theme === "dark"
