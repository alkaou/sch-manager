...
const EventForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let savedEvent;
      if (isEditing) {
        savedEvent = await updateEvent(event.id, formData, database, setFlashMessage, language);
      } else {
        savedEvent = await createEvent(formData, database, setFlashMessage, language);
      }
  ...
  const getInputClasses = (hasError = false) => {
    const baseClasses = "w-full px-4 py-3 rounded-lg border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500";
        ...
        >
          ...
          <div className={`flex items-center justify-between p-6 border-b ${
            ...
            <h2 className="text-2xl font-bold flex items-center gap-3">
              ...
              {isEditing ? translate('edit_event', language) : translate('add_event', language)}
            </h2>
            <motion.button
