    ...
    >
      ...
      >
        ...
        <div className={`flex justify-between items-center p-4 border-b ${borderColor}`}>
          <h2 className={`text-xl font-semibold ${textClass}`}>{t("add_students_to_list")}</h2>
          <motion.button
            onClick={onClose}
          ...
          <div className="mb-4 flex justify-between items-center">
            <div className="flex items-center">
              ...
              <span className={`${textClass}`}>
                {selectedStudents.length} {t("students_selected")} {filteredStudents.length} {t("displayed")}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <p className={`text-sm ${theme === "dark" ? "text-gray-300" : "text-gray-500"}`}>
                        {getClasseName(student.classe) || t("no_assigned_class")}
                      </p>
                      {student.matricule && (
                        <p className={`text-sm ${theme === "dark" ? "text-gray-300" : "text-gray-500"}`}>
            ...
            {filteredStudents.length === 0 && (
              <div className={`col-span-3 text-center p-8 ${textClass}`}>
                <p className="text-lg">{t("no_students_found")}</p>
                <p className="opacity-75">{t("try_different_filters")}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className={`p-4 border-t ${borderColor} flex justify-end space-x-3`}>
          ...
          >
            ...
            {t("add_count_students", { count: selectedStudents.length })}
          </motion.button>
        </div>
      </motion.div>
    </motion.div>
