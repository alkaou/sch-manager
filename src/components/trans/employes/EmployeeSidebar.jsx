  ...
  return (
    ...
    >
      <div className="p-2 sm:p-3 md:p-4 border-b border-gray-200 flex justify-between items-center">
        <h2 className={`font-bold text-xs xs:text-sm md:text-lg ${_text_color} truncate`}>{translate("positions")}</h2>
        <motion.button
          onClick={() => setShowAddPositionForm(true)}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
          className={`${buttonColor} p-1 rounded-full`}
          title={translate("add_position")}
        >
          <PlusCircle size={18} />
        </motion.button>
      </div>
                      ...
                      <div className="flex items-center">
                        ...
                        {position.description && (
                          <motion.button
                            ...
                            title={expandedDescriptions[position.id] ? translate("hide_description") : translate("show_description")}
                          >
                            <Info size={14} />
                          </motion.button>
                        )}
                    ...
                    <AnimatePresence>
                      {expandedDescriptions[position.id] && position.description && (
                        ...
                        >
                          ...
                          `}>
                            <span className={`text-xs mb-0 sm:mb-1 font-medium ${_text_color} opacity-70`}>{translate("description")}</span>
                          </div>
      ...
      <AnimatePresence>
        {confirmDelete && (
          ...
          >
            ...
            >
              <h3 className={`text-base sm:text-xl font-semibold mb-2 sm:mb-4 ${_text_color}`}>{translate("confirm_deletion")}</h3>
              <p className={`${_text_color} mb-4 sm:mb-6 text-sm sm:text-base`}>
                {translate("confirm_delete_position").replace("{position}", confirmDelete.name)}
                {confirmDelete.employeeCount > 0 && (
                  <span className="text-red-500 block mt-2 font-semibold text-xs sm:text-sm">
                    {translate("warning")}: {translate("employees_with_position").replace("{count}", confirmDelete.employeeCount)}
                    <br />
                    - {translate("employees_only_this_position")}
                    - {translate("position_removed_from_others")}
                  </span>
                )}
              </p>
