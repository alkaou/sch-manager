    ...
    >
      <div className="flex flex-col sm:flex-row flex-wrap items-center gap-2 sm:gap-4">
        ...
        <div className="w-full sm:flex-1 mb-2 sm:mb-0">
          <h2 className={`text-lg sm:text-xl font-bold ${_text_color}`}>
            {translate("employees")}: {positionName}
          </h2>
        </div>
        
        {/* Search */}
        <div className="w-full sm:w-auto sm:flex-1">
          <div className={`flex items-center px-2 sm:px-3 py-1.5 sm:py-2 rounded-md ${inputBgColor}`}>
            <Search size={16} className={`${_text_color} opacity-60 mr-2`} />
            <input
              type="text"
              placeholder={translate("search_employees")}
      ...
      {(filters.searchTerm || filters.status !== 'All') && (
        <div className="mt-3 flex flex-wrap gap-2 items-center">
          <span className={`text-xs sm:text-sm ${_text_color}`}>{translate("active_filters")}:</span>
          ...
          {filters.searchTerm && (
            ...
            >
              <span>{translate("search")}: {filters.searchTerm}</span>
              <button 
          ...
          {filters.status !== 'All' && (
            <motion.div 
              ...
              className={`px-2 py-1 rounded-md ${activeFilterBg} text-xs sm:text-sm ${_text_color} flex items-center`}
            >
              <span>{translate("status")}: {filters.status === 'actif' ? translate("active") : translate("inactive")}</span>
              <button 
