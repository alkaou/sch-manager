...
const PayementsMonthlyClass = ({ db, theme, text_color }) => {
    ...
    const getCurrentMonthDisplay = () => {
        ...
        return t("month_not_selected");
    };

    // Formater la date pour l'affichage
    const formatDate = (dateString) => {
        const date = new Date(dateString);
        return date.toLocaleDateString(language === "Anglais" ? "en-US" : "fr-FR", { 
            day: 'numeric', 
            month: 'long', 
            year: 'numeric' 
        });
                    ...
                    <div className="mb-6">
                        <div className={`p-4 ${headerBgColor} rounded-lg shadow-md`}>
                            <div className="flex items-center mb-3">
                                ...
                                <h3 className={`font-semibold ${_text_color}`}>{t("select_payment_period")}</h3>
                            </div>
