...
const PayementsYearlyStatistique = ({ db, theme, app_bg_color, text_color }) => {
  ...
  const formatCurrency = (amount) => {
    return new Intl.NumberFormat(language === "Anglais" ? "en-US" : "fr-FR", {
      style: 'currency',
      currency: 'XAF',
  ...
  const getTrendChartData = () => {
    // Get months in current language
    const months = [
      t('month_short_jan'), t('month_short_feb'), t('month_short_mar'), 
      t('month_short_apr'), t('month_short_may'), t('month_short_jun'), 
      t('month_short_jul'), t('month_short_aug'), t('month_short_sep'), 
      t('month_short_oct'), t('month_short_nov'), t('month_short_dec')
    ];
  ...
  const getComparisonMetricLabel = () => {
    switch (comparisonMetric) {
      ...
      case 'expectedRevenue':
        return t('expected_revenue');
      case 'paymentPercentage':
        return t('payment_rate');
      case 'registrationFees':
        return t('registration_fees');
      case 'monthlyFees':
        return t('monthly_fees');
      case 'yearlyFees':
        return t('yearly_fees');
      default:
        return t('total_revenue');
    }
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h2 className={`text-2xl font-bold ${text_color}`}>{t("yearly_payment_statistics")}</h2>

        <div className="flex flex-wrap gap-3">
          <div className="flex flex-col">
            <div className="flex items-center mb-2">
              <Calendar className={`h-5 w-5 mr-2 ${text_color}`} />
              <span className={`text-sm font-medium ${text_color}`}>{t("school_years")}</span>
            </div>
            <div className="flex flex-wrap gap-2 max-w-md">
              {availableYears.map(year => (
                <button
                  ...
                  `}
                >
                  {year.label.replace(`${t("school_year")} `, '')}
                  {selectedYears.includes(year.key) && (
            ...
            {selectedYears.length === 0 && (
              <p className={`text-xs mt-1 text-red-500`}>
                {t("please_select_at_least_one_year")}
              </p>
            )}
            {selectedYears.length === 4 && (
              <p className={`text-xs mt-1 text-amber-500`}>
                {t("maximum_four_years_selectable")}
              </p>
            )}
          </div>
          ...
          <div className="flex items-center">
            ...
            >
              <option value="totalRevenue">{t("total_revenue")}</option>
              <option value="expectedRevenue">{t("expected_revenue")}</option>
              <option value="paymentPercentage">{t("payment_rate")}</option>
              <option value="registrationFees">{t("registration_fees")}</option>
              <option value="monthlyFees">{t("monthly_fees")}</option>
              <option value="yearlyFees">{t("yearly_fees")}</option>
            </select>
          </div>
          ...
          <div className="flex items-center">
            ...
            >
              ...
              <option value="line">{t("linear_chart")}</option>
              <option value="polarArea">{t("polar_chart")}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className={`${cardBgColor} rounded-lg shadow-lg p-8 text-center ${textColorClass}`}>
          ...
          <h3 className="text-xl font-bold mb-2">{t("no_data_available")}</h3>
          <p className="text-gray-500">
            {t("please_select_at_least_one_school_year_to_display")}
          </p>
        </div>
      ) : (
        <motion.div
            ...
            >
              ...
              <div className={`${cardBgColor} rounded-lg p-6 shadow-lg border-l-4 border-blue-500 overflow-hidden`}>
                <div className="flex items-center mb-4">
                  ...
                  <div className="min-w-0">
                    <h3 className={`text-sm font-medium ${textColorClass} opacity-70`}>{t("total_revenue")}</h3>
                  </div>
                </div>
                <div className="mt-1">
                  <div className="mb-1">
                  ...
                  <p className={`text-xs ${textColorClass} opacity-60`}>
                    {t("school_year")} {yearlyData[0]?.label}
                  </p>
                </div>
              </div>
              ...
              <div className={`${cardBgColor} rounded-lg p-6 shadow-lg border-l-4 border-green-500 overflow-hidden`}>
                <div className="flex items-center mb-4">
                  ...
                  <div className="min-w-0">
                    <h3 className={`text-sm font-medium ${textColorClass} opacity-70`}>{t("expected_revenue")}</h3>
                  </div>
                </div>
                <div className="mt-1">
                  <div className="mb-1">
                  ...
                  <p className={`text-xs ${textColorClass} opacity-60`}>
                    {t("financial_goal_for")} {yearlyData[0]?.label}
                  </p>
                </div>
              </div>
              ...
              <div className={`${cardBgColor} rounded-lg p-6 shadow-lg border-l-4 border-purple-500 overflow-hidden`}>
                <div className="flex items-center mb-4">
                  ...
                  <div className="min-w-0">
                    <h3 className={`text-sm font-medium ${textColorClass} opacity-70`}>{t("payment_rate")}</h3>
                    <p className="text-2xl font-bold text-purple-500 break-words">{yearlyData[0]?.paymentPercentage || 0}%</p>
                  </div>
                </div>
                <div className="mt-2">
                  <p className={`text-xs ${textColorClass} opacity-60`}>
                    {t("percentage_of_received_revenue")}
                  </p>
                </div>
              </div>
              ...
              <div className={`${cardBgColor} rounded-lg p-6 shadow-lg border-l-4 ${yearOverYearGrowth >= 0 ? 'border-amber-500' : 'border-red-500'} overflow-hidden`}>
                <div className="flex items-center mb-4">
                  ...
                  <div className="min-w-0">
                    <h3 className={`text-sm font-medium ${textColorClass} opacity-70`}>{t("annual_growth")}</h3>
                ...
                <div className="mt-2">
                  <p className={`text-xs ${textColorClass} opacity-60`}>
                    {selectedYears.length > 1
                      ? `${t("compared_to")} ${availableYears.find(y => y.key === selectedYears[1])?.label}`
                      : t("select_previous_year_to_compare")}
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          >
            ...
            <div className={`${cardBgColor} rounded-lg shadow-lg p-6`}>
              <h3 className={`text-lg font-semibold mb-4 ${textColorClass}`}>
                {t("comparison_of")} {getComparisonMetricLabel()} {t("by_school_year")}
              </h3>
              <div className="h-80">
                {chartType === 'bar' && (
            ...
            <div className={`${cardBgColor} rounded-lg shadow-lg p-6`}>
              <h3 className={`text-lg font-semibold mb-4 ${textColorClass}`}>
                {t("monthly_evolution_of_payment_rate")}
              </h3>
              <div className="h-80">
                  ...
                  <div key={cls.id} className={`p-3 rounded-lg ${cardHoverEffect} border ${borderColor}`}>
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{cls.studentCount} {t("students")}</p>
                      </div>
