...
const PayementsYearlyClass = ({ db, theme, app_bg_color, text_color }) => {
            ...
            db.paymentSystems.forEach(system => {
                ...
                if (!schoolYears.has(schoolYearKey)) {
                    schoolYears.set(schoolYearKey, {
                        ...
                        startYear,
                        endYear,
                        label: `${t("school_year")} ${startYear}-${endYear}`,
                        systems: []
                    });
                }
    ...
    const formatCurrency = (amount) => {
        return new Intl.NumberFormat(language === "Anglais" ? "en-US" : "fr-FR").format(amount) + " FCFA";
    };
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-4 sm:mb-6">
                <h2 className={`text-lg sm:text-xl md:text-2xl font-bold ${text_color}`}>{t("annual_budget_by_class")}</h2>
                ...
                <div className="flex items-center">
                    ...
                    >
                        {availableSchoolYears.length > 0 ? (
                            ...
                            <option value="">{t("no_school_years_available")}</option>
                        )}
                    </select>
                </div>
            </div>
                ...
                <>
                    ...
                    >
                        ...
                        >
                            <div className="flex items-center mb-2 sm:mb-4">
                                ...
                                <div>
                                    <h3 className={`text-xs sm:text-sm font-medium ${textColorClass} opacity-70`}>{t("total_expected_budget")}</h3>
                            ...
                            <div className="mt-1 sm:mt-2">
                                <p className={`text-2xs sm:text-xs ${textColorClass} opacity-60`}>
                                    {t("annual_budget_for_all_classes")}
                                </p>
                            </div>
                        </motion.div>
                        ...
                        >
                            <div className="flex items-center mb-2 sm:mb-4">
                                ...
                                <div>
                                    <h3 className={`text-xs sm:text-sm font-medium ${textColorClass} opacity-70`}>{t("total_amount_received")}</h3>
                                    <p className="text-base sm:text-xl md:text-2xl font-bold text-green-500">{formatCurrency(totalReceived)}</p>
                                </div>
                            </div>
                            <div className="mt-1 sm:mt-2">
                                <p className={`text-2xs sm:text-xs ${textColorClass} opacity-60`}>
                                    {t("total_amount_received_so_far")}
                                </p>
                            </div>
                        </motion.div>
                        ...
                        >
                            <div className="flex items-center mb-2 sm:mb-4">
                                ...
                                <div>
                                    <h3 className={`text-xs sm:text-sm font-medium ${textColorClass} opacity-70`}>{t("pending_amount")}</h3>
                                    <p className="text-base sm:text-xl md:text-2xl font-bold text-red-500">{formatCurrency(totalExpected - totalReceived)}</p>
                                </div>
                            </div>
                            <div className="mt-1 sm:mt-2">
                                <p className={`text-2xs sm:text-xs ${textColorClass} opacity-60`}>
                                    {t("pending_amount_so_far")}
                                </p>
                            </div>
                        </motion.div>
                        ...
                        >
                            <div className="flex items-center mb-2 sm:mb-4">
                                ...
                                <div>
                                    <h3 className={`text-xs sm:text-sm font-medium ${textColorClass} opacity-70`}>{t("recovery_rate")}</h3>
                    ...
                    {yearlyData.length === 0 ? (
                        <div className={`${cardBgColor} rounded-lg shadow-lg p-4 sm:p-6 md:p-8 text-center ${textColorClass}`}>
                            ...
                            <h3 className="text-base sm:text-lg md:text-xl font-bold mb-1 sm:mb-2">{t("no_data_available")}</h3>
                            <p className="text-xs sm:text-sm text-gray-500">
                                {t("no_class_configured_or_no_students_enrolled")}
                            </p>
                        </div>
                    ) : (
                        <motion.div
                                    ...
                                    <div className="p-3 sm:p-4 md:p-5">
                                        <div className="mb-3 sm:mb-4">
                                            <div className="flex justify-between mb-0.5 sm:mb-1">
                                                <span className={`${textColorClass} text-xs sm:text-sm`}>{t("payment_progress")}</span>
                                        ...
                                        <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-3 sm:mb-4">
                                            <div className={`p-2 sm:p-3 rounded-lg ${headerBgColor}`}>
                                                <h4 className={`text-2xs sm:text-xs font-bold ${textColorClass} mb-0.5 sm:mb-1`}>{t("monthly_fees")} ({data.monthCount} {t("months")})</h4>
                                                <p className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                    {formatCurrency(data.monthlyTotal)}
                                                </p>
                                            </div>
                                            <div className={`p-2 sm:p-3 rounded-lg ${headerBgColor}`}>
                                                <h4 className={`text-2xs sm:text-xs font-bold ${textColorClass} mb-0.5 sm:mb-1`}>{t("registration_fees")}</h4>
                                                <p className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                    {formatCurrency(data.registrationTotal)}
                                                </p>
                                                {data.newStudentCount > 0 && (
                                                    <p className={`text-2xs sm:text-xs ${textColorClass} opacity-70`}>
                                                        ({data.newStudentCount} {t("new_students")})
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-1 sm:space-y-2 pt-2 border-t border-gray-200">
                                            <div className="flex justify-between">
                                                <span className={`${textColorClass} text-xs sm:text-sm`}>{t("expected_amount")}:</span>
                                                <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                    {formatCurrency(data.expectedAmount)}
                                                </span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className={`${textColorClass} text-xs sm:text-sm`}>{t("received_amount")}:</span>
                                                <span className="text-xs sm:text-sm font-bold text-green-600">
                                                    {formatCurrency(data.receivedAmount)}
                                                </span>
                                            </div>
                                            <div className="flex justify-between pt-1 sm:pt-2 border-t border-gray-200">
                                                <span className={`${textColorClass} text-xs sm:text-sm`}>{t("remaining_amount")}:</span>
                                ...
                                >
                                    <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-3 sm:p-4 md:p-6 text-white relative">
                                        ...
                                        <div className="flex items-center mt-1 sm:mt-2">
                                            ...
                                            <span className="text-xs sm:text-sm">{selectedClass.studentCount} {t("students")}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-3 sm:p-4 md:p-6">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 mb-3 sm:mb-4 md:mb-6">
                                            <div className={`p-2 sm:p-3 md:p-4 rounded-lg ${headerBgColor}`}>
                                                <h3 className={`text-sm sm:text-base md:text-lg font-bold ${textColorClass} mb-2 sm:mb-3`}>{t("fees_details")}</h3>
                                                <div className="space-y-2 sm:space-y-3">
                                                    <div className="flex justify-between">
                                                        <span className={`${textColorClass} text-xs sm:text-sm`}>{t("monthly_fee")}:</span>
                                                        <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                            {formatCurrency(selectedClass.monthlyFee)}
                                                        </span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className={`${textColorClass} text-xs sm:text-sm`}>{t("number_of_months")}:</span>
                                                        <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                            {selectedClass.monthCount}
                                                        </span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className={`${textColorClass} text-xs sm:text-sm`}>{t("registration_fee")}:</span>
                                                        <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                            {formatCurrency(selectedClass.registrationFee)}
                                                        </span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className={`${textColorClass} text-xs sm:text-sm`}>{t("new_students")}:</span>
                                                        <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                            ...
                                            <div className={`p-2 sm:p-3 md:p-4 rounded-lg ${headerBgColor}`}>
                                                <h3 className={`text-sm sm:text-base md:text-lg font-bold ${textColorClass} mb-2 sm:mb-3`}>{t("financial_summary")}</h3>
                                                <div className="space-y-2 sm:space-y-3">
                                                    <div className="flex justify-between">
                                                        <span className={`${textColorClass} text-xs sm:text-sm`}>{t("total_monthly_budget")}:</span>
                                                        <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                            {formatCurrency(selectedClass.monthlyTotal)}
                                                        </span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className={`${textColorClass} text-xs sm:text-sm`}>{t("total_registration_budget")}:</span>
                                                        <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                            {formatCurrency(selectedClass.registrationTotal)}
                                                        </span>
                                                    </div>
                                                    <div className="flex justify-between pt-1 sm:pt-2 border-t border-gray-300">
                                                        <span className={`${textColorClass} text-xs sm:text-sm`}>{t("total_expected_budget")}:</span>
                                                        <span className={`${textColorClass} text-xs sm:text-sm font-bold`}>
                                                            {formatCurrency(selectedClass.expectedAmount)}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className={`p-2 sm:p-3 md:p-4 rounded-lg ${headerBgColor} mb-3 sm:mb-4 md:mb-6`}>
                                            <h3 className={`text-sm sm:text-base md:text-lg font-bold ${textColorClass} mb-2 sm:mb-3`}>{t("payment_progress")}</h3>
                                            <div className="mb-3 sm:mb-4">
                                                <div className="flex justify-between mb-1 sm:mb-2">
                                                    <span className={`${textColorClass} text-xs sm:text-sm`}>{t("received_amount")}:</span>
                                                    <span className="text-xs sm:text-sm font-bold text-green-600">
                                                        {formatCurrency(selectedClass.receivedAmount)}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between mb-1 sm:mb-2">
                                                    <span className={`${textColorClass} text-xs sm:text-sm`}>{t("remaining_amount")}:</span>
                                                    <span className={`text-xs sm:text-sm font-bold ${selectedClass.remainingAmount > 0 ? 'text-red-500' : 'text-green-600'}`}>
                                                        {formatCurrency(selectedClass.remainingAmount)}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between mb-1 sm:mb-2">
                                                    <span className={`${textColorClass} text-xs sm:text-sm`}>{t("recovery_rate")}:</span>
