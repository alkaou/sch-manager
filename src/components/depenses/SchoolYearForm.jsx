...
const SchoolYearForm = ({
  ...
  useEffect(() => {
    if (isEditMode && schoolYear) {
      ...
      if (today.getMonth() >= 8) {
        // If we're already in September or later
      ...
      setFormData({
        title: `${t("school_year_title_template")
          .replace("YEAR1", startDate.getFullYear())
          .replace("YEAR2", endDate.getFullYear())}`,
        description: `${t("school_year_description_template")
          .replace("YEAR1", startDate.getFullYear())
          .replace("YEAR2", endDate.getFullYear())}`,
  ...
  const validateForm = () => {
    ...
    if (!formData.title.trim()) {
      ...
      formData.title.trim().length > 150
    ) {
    ...
    const duplicateCheck = schoolYears.find(
      (year) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        await window.electron.saveDatabase(updatedDb);
        await setSchoolYears(updatedSchoolYears);
      ...
      setFlashMessage({
        message: isEditMode ? t("error_saving") : t("error_saving"),
        type: "error",
        duration: 3000,
      });
    } finally {
  ...
  const inputFocusBorderColor =
    theme === "dark" ? "focus:border-blue-500" : "focus:border-blue-500";
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h2 className="text-2xl font-bold">
              {isEditMode ? t("edit_school_year") : t("add_school_year")}
            </h2>
          </div>
          <button
            onClick={onCancel}
        ...
        <div className="space-y-6">
          ...
          <motion.div variants={controlVariants}>
            ...
            <input
              ...
              className={`w-full px-4 py-2 rounded-lg ${inputBgColor} ${inputBorderColor} border ${inputFocusBorderColor} focus:ring-2 focus:ring-blue-500 transition-colors ${
          ...
          >
            <Info
              size={18}
              className={`${mutedTextColor} flex-shrink-0 mt-0.5 mr-3`}
            />
          ...
          >
            <AlertTriangle
              size={18}
              className={`text-amber-500 flex-shrink-0 mt-0.5 mr-3`}
            />
        ...
        >
          ...
          <button
            ...
            className={`px-5 py-2 rounded-lg flex items-center text-white ${buttonColor} transition-colors ${
              isSubmitting || (!formTouched && isEditMode)
