...
const StudentListAddStudents = ({
  ...
  const levels = db?.classes
    ? [...new Set(db.classes.map((c) => c.level))]
    : [];
  ...
  const filteredStudents = students.filter((student) => {
    ...
    const levelMatch =
      ...
      (() => {
        ...
        const studentLevel = student.classe
          ? parseInt(student.classe.split(" ")[0])
          : null;
      ...
      >
        ...
        >
          <h2 className={`text-xl font-semibold ${textClass}`}>
            {translate("add_students_to_list", language)}
          </h2>
          <motion.button
            onClick={onClose}
        ...
        <div className={`p-4 border-b ${borderColor} flex flex-wrap gap-4`}>
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            >
              ...
              <input
                ...
                placeholder={translate("search_student", language)}
          ...
          <div className="w-64">
            ...
            >
              <option value="all">{translate("all_classes", language)}</option>
              {db?.classes
          ...
          <div className="w-64">
            <select
              ...
              className={`w-full p-2 border ${borderColor} rounded-lg ${inputBgColor} ${textClass}`}
            >
              <option value="all">{translate("all_grades", language)}</option>
              {levels
                .sort((a, b) => a - b)
                .map((level) => (
                  <option key={level} value={level}>
                    {translate("level", language)} {level}
                  </option>
                ))}
            </select>
          </div>
          ...
          <div className="mb-4 flex justify-between items-center">
            <div className="flex items-center">
              ...
              <span className={`${textClass}`}>
                ...
                {translate("students_selected", language)}{" "}
                {filteredStudents.length} {translate("displayed", language)}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      >
                        {getClasseName(student.classe, language) ||
                          translate("no_assigned_class", language)}
                      </p>
                      {student.matricule && (
                        <p
                          className={`text-sm ${
            ...
            {filteredStudents.length === 0 && (
              <div className={`col-span-3 text-center p-8 ${textClass}`}>
                <p className="text-lg">
                  {translate("no_students_found", language)}
                </p>
        ...
        >
          ...
          <motion.button
            ...
            className={`${
              selectedStudents.length > 0 ? buttonSuccess : "bg-green-400"
          ...
          >
            ...
            {translate("add", language)} {selectedStudents.length}{" "}
