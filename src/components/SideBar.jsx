...
import { 
  Home, Settings, Palette, ChevronDown, Menu, Edit2, Check, 
  Layout, FileText, Users, DollarSign, BarChart, Database as DatabaseIcon 
} from "lucide-react";
...
const SideBar = ({ setIsOpenPopup, school_name, school_short_name, text_color, setIsShowParameters, setisShowBgColorSelector, activeSideBarBtn, setActiveSideBarBtn, db, refreshData }) => {
  ...
  // États pour le mode édition et valeurs modifiables
  ...
  // Méthode générique de navigation qui met à jour le bouton actif et navigue
          ...
          >
            ...
            {showOptionsNames && (
              <div className="ml-2 text-center items-center justify-center rounded-full h-8 w-8 border border-gray-300 hover:bg-green-200">
                <Edit2
                  size={20}
                  className={`cursor-pointer ml-1 pb-1 mt-2 ${side_bar_text_color}`}
        ...
        <nav className="flex flex-col space-y-4">
          <SidebarItem
            ...
            onClick={() => {
              ...
              handleNavigation("/started_page", 1);
            }}
          ...
          <div>
            ...
            >
              <Settings size={24} />
              {showOptionsNames && <span className={`ml-4 flex-1 ${side_bar_text_color}`}>{live_language.setting_text}</span>}
              <ChevronDown size={18} className={`transition-transform ${isSettingsOpen ? "rotate-180" : ""}`} />
            </button>
          </div>
        </nav>

        {/* Nouveaux boutons de navigation placés en bas */}
        <div className="mt-auto">
          <nav className="flex flex-col space-y-4">
            <SidebarItem
              icon={<Layout size={24} />}
              text={showOptionsNames ? "Compositions" : ""}
              isOpen={isOpen}
              onClick={() => handleNavigation("/compositions", 4)}
              active_class={activeSideBarBtn === 4 ? "text-white bg-gray-800" : ""}
            />
            <SidebarItem
              icon={<FileText size={24} />}
              text={showOptionsNames ? "Bulletins" : ""}
              isOpen={isOpen}
              onClick={() => handleNavigation("/bulletins", 5)}
              active_class={activeSideBarBtn === 5 ? "text-white bg-gray-800" : ""}
            />
            <SidebarItem
              icon={<Users size={24} />}
              text={showOptionsNames ? "Liste des élèves" : ""}
              isOpen={isOpen}
              onClick={() => handleNavigation("/liste_eleves", 6)}
              active_class={activeSideBarBtn === 6 ? "text-white bg-gray-800" : ""}
            />
            <SidebarItem
              icon={<DollarSign size={24} />}
              text={showOptionsNames ? "Payements" : ""}
              isOpen={isOpen}
              onClick={() => handleNavigation("/payements", 7)}
              active_class={activeSideBarBtn === 7 ? "text-white bg-gray-800" : ""}
            />
            <SidebarItem
              icon={<BarChart size={24} />}
              text={showOptionsNames ? "Statistiques" : ""}
              isOpen={isOpen}
              onClick={() => handleNavigation("/statistiques", 8)}
              active_class={activeSideBarBtn === 8 ? "text-white bg-gray-800" : ""}
            />
            <SidebarItem
              icon={<DatabaseIcon size={24} />}
              text={showOptionsNames ? "Base de donnée" : ""}
              isOpen={isOpen}
              onClick={() => handleNavigation("/database", 9)}
              active_class={activeSideBarBtn === 9 ? "text-white bg-gray-800" : ""}
            />
          </nav>
        </div>
      </motion.aside>
