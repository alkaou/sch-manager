...
const BulletinComponent = ({
  ...
  useEffect(() => {
    ...
    const studentAverage = parseFloat(
    ...
    const position =
      averages.findIndex((s) => s.average === studentAverage) + 1;

    // Si plusieurs élèves ont la même moyenne, ajouter "EX aequo"
    const _student_position =
      position === 1
        ? student.sexe === "F"
          ? "1ère"
          : "1er"
  ...
  const mainCoefficients = mainSubjects.reduce(
    (total, subject) => total + subject.coefficient,
    0
  );
  ...
  const showBllt_1 = type !== "type2" && isSconcycle ? true : false;
