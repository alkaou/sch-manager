...
const PayementsTabs = ({
  ...
  return (
    <div className={`${tabsBgColor} border-b-2 mt-2 sm:mt-3 md:mt-5 ${borderColor} shadow-md`}>
      <div className="container mx-auto px-1 sm:px-2">
        <div className="flex overflow-hidden scrollbar-thin scrollbar-custom">
          {tabs.map((tab) => (
            <motion.button
              key={tab.id}
              onClick={() => {
              ...
              className={`flex items-center space-x-1 px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 md:py-3 text-xs sm:text-sm font-medium transition-colors relative ${activeTab === tab.id
                ? `${activeTextColor}`
            ...
            >
              ...
              {activeTab === tab.id && (
                <motion.div
                  className={`absolute bottom-0 left-0 w-full h-0.5 sm:h-1 ${activeTabBgColor}`}
                  layoutId="activeTab"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
