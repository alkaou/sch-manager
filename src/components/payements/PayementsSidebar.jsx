  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`backdrop-filter backdrop-blur-sm bg-opacity-90 p-3 sm:p-4 md:p-5 border-b ${borderColor}`}
        >
          <h2
            className={`text-base sm:text-lg md:text-xl font-bold ${_text_color} mb-2 sm:mb-3 md:mb-4 flex items-center`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 mr-1 sm:mr-2"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <div className="flex-1 overflow-hidden p-2 sm:p-3 md:p-4">
          {!hasPaymentSystems ? (
            ...
            <div
              className={`space-y-3 sm:space-y-4 md:space-y-5 ${sidebarBgColor}`}
            >
              {filteredPaymentSystems.map((system) => {
                // Get classes for this payment system
                const _systemClasses =
                  db?.classes?.filter(
                    (cls) => system.classes && system.classes.includes(cls.id)
                  ) || [];
                const systemClasses = _systemClasses.sort(
                  (a, b) => b.level - a.level
                );
                ...
                return (
                  <div
                    key={system.id}
                    className={`rounded-xl overflow-hidden ${cardShadow} transition-all duration-300 ${
                      expandedSystems[system.id]
                        ? "border-l-4 border-blue-500"
                        : `${
                            isWhiteOrGray || theme === "dark"
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        className={`font-bold text-sm sm:text-base md:text-lg mb-0.5 sm:mb-1 ${_text_color} ${
                          expandedSystems[system.id]
                        }`}
                      >
                        {system.name}
                      ...
                      <motion.button
                        ...
                        py-1.5 sm:py-2 md:py-2.5 rounded-lg ${
                          expandedSystems[system.id] ? "bg-opacity-10" : ""
                        }
                      `}
                      >
                        <div className="flex items-center mr-1">
                          <div
                            className={`mr-2 sm:mr-3 p-1.5 sm:p-2 md:p-2.5 rounded-full ${
                              expandedSystems[system.id]
                                ? theme === "dark"
                          ...
                          <div className="flex flex-col">
                            <span
                              className={`text-2xs sm:text-xs ${_text_color} font-medium`}
                            >
                    ...
                    <AnimatePresence>
                      {expandedSystems[system.id] && (
                        ...
                        >
                          <div
                            className={`p-2 sm:p-3 md:p-4 space-y-1.5 sm:space-y-2 md:space-y-2.5 ${
                              theme === "dark"
                          ...
                          >
                            {systemClasses.length === 0 ? (
                              <div
                                className={`text-center ${_text_color} opacity-80 py-3 sm:py-4 md:py-5 flex flex-col items-center rounded-lg ${app_bg_color}`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  className="h-6 w-6 sm:h-8 sm:w-8 md:h-10 md:w-10 text-gray-400 mb-2 sm:mb-3"
                                  fill="none"
                                  viewBox="0 0 24 24"
                                  stroke="currentColor"
                                >
                              ...
                              <div className="grid grid-cols-1 gap-2 sm:gap-2.5 md:gap-3">
                                {systemClasses.map((cls) => {
                                  const className =
                                    `${cls.level} ${cls.name}`.trim();
                                  // console.log(className);
                                  const isActive =
                                    selectedClass &&
                                    selectedClass.id === cls.id &&
                                    selectedPaymentSystem.id === system.id;
                                  const studentCount = countStudentsInClass(
                                    className,
                                    cls.id,
                                    system.id
                                  );
                                  ...
                                  return (
                                    ...
                                    >
                                      <button
                                        ...
                                        className={`w-full text-left px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 md:py-3.5 rounded-lg flex justify-between items-center transition-all duration-200 ${
                                          isActive
                                      ...
                                      >
                                        <div className="flex items-center">
                                          <div
                                            className={`mr-1.5 sm:mr-2 md:mr-3 p-1 sm:p-1.5 md:p-2 rounded-full ${
                                              isActive
                                        ...
                                        <div
                                          className={`flex border border-2 items-center px-2 sm:px-2.5 md:px-3 py-1 sm:py-1.5 rounded-full ${
                                            isActive
                                        ...
                                        >
                                          <span className="text-2xs sm:text-xs md:text-sm font-semibold">
                                            {studentCount}
                                          </span>
          ...
          <div className="p-2 sm:p-3 md:p-4 border-t border-gray-700 bg-opacity-90">
            ...
            >
              <div className="flex items-center justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 mr-1 sm:mr-2"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
