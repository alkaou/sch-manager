...
const EmployeeSidebar = ({
  ...
  const descriptionVariants = {
    ...
    visible: { opacity: 1, height: "auto", marginTop: 8, transition: { duration: 0.3 } },
  };
  ...
  return (
    ...
    >
      <div className="p-2 sm:p-3 md:p-4 border-b border-gray-200 flex justify-between items-center">
        <h2 className={`font-bold text-xs xs:text-sm md:text-lg ${_text_color} truncate`}>
      ...
      <div className="flex-1 overflow-y-auto p-1 sm:p-2 scrollbar-custom">
        <AnimatePresence>
          {filteredPositions.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`text-center p-2 sm:p-4 ${_text_color} opacity-70 text-xs sm:text-sm`}>
            ...
            <ul className="space-y-1">
              {filteredPositions.map((position) => (
                <motion.li
                  ...
                  className={`relative rounded-md overflow-hidden ${selectedPosition === position.name
                    ? theme === "dark"
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <div
                      ...
                      tabIndex={0}
                      onKeyPress={(e) => {
                        if (e.key === "Enter") setSelectedPosition(position.name);
                      }}
                    ...
                    >
                      <div className="flex items-center space-x-1 sm:space-x-2 truncate">
                        <Briefcase
                          size={16}
                          className={
                            selectedPosition === position.name ? "text-blue-500" : ""
                          }
                        />
                        <span
                          className={`text-xs sm:text-sm ${selectedPosition === position.name ? "font-semibold" : ""
                            } truncate`}
                        >
                          ...
                          {hoveredPosition === position.id && position.name !== "Professeurs" && (
                            ...
                            >
                              <motion.button
                                ...
                                whileTap={{ scale: 0.9 }}
                                onClick={(e) => { e.stopPropagation(); setPositionToEdit(position); }}
                              ...
                              <motion.button
                                whileHover={{ scale: 1.2 }}
                                whileTap={{ scale: 0.9 }}
                                onClick={(e) => { e.stopPropagation(); setConfirmDelete(position); }}
                    ...
                    <AnimatePresence>
                      {expandedDescriptions[position.id] && position.description && (
                        <motion.div variants={descriptionVariants} initial="hidden" animate="visible" exit="hidden" className={`mx-2 sm:mx-3 mb-2 rounded ${descriptionBgColor} border ${borderColor} text-sm`}>
                          <div className={`flex items-center border-b border-b-1 p-1 sm:p-2 ${theme === "dark" ? "" : "bg-gray-200"}`}>
                            <span className={`text-xs mb-0 sm:mb-1 font-medium ${_text_color} opacity-70`}>
      ...
      <AnimatePresence>
        {confirmDelete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className={`${theme === "dark" ? "bg-gray-800" : "bg-white"} p-4 sm:p-6 rounded-lg shadow-xl max-w-md mx-auto`}>
              <h3 className={`text-base sm:text-xl font-semibold mb-2 sm:mb-4 ${_text_color}`}>
                {translate("confirm_delete", language)}
              </h3>
              <p className={`${_text_color} mb-4 sm:mb-6 text-sm sm:text-base`}>
                {translate("delete_poste_alert_message", language)} "{confirmDelete.name}" ?
                {confirmDelete.employeeCount > 0 && (
                  <span className="text-red-500 block mt-2 font-semibold text-xs sm:text-sm">
                    {translate("attention", language)} : {confirmDelete.employeeCount} {translate("msg_occupation", language)}<br />
                    {translate("msg_delete_poste_info", language)}
                  </span>
                )}
              </p>
              <div className="flex justify-end space-x-3">
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setConfirmDelete(null)} className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-md text-sm ${theme === "dark" ? "bg-gray-700 hover:bg-gray-600" : "bg-gray-200 hover:bg-gray-300"} ${_text_color}`}> {translate("cancel", language)} </motion.button>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handleDeletePosition(confirmDelete.id)} disabled={false} className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-md text-sm text-white bg-red-500 hover:bg-red-600"> {translate("delete", language)} </motion.button>
              </div>
            </motion.div>
          </motion.div>
        )}
