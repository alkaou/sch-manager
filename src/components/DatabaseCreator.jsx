...
function DatabaseCreator({ setIsOpenPopup }) {
	...
	const createNewDatabase = () => {
		...
		const creating_or_updating_message = isForUpdate ? live_language.update_db_message : live_language.creating_db_message;
		const loading_updating_message = live_language.loading_message;
		const loading_patient_message = isForUpdate ? live_language.update_loading_patient_message : live_language.loading_patient_message;
		const finalisation_message = isForUpdate ? live_language.update_finalisation_db_message : live_language.created_db_message;
		const finished_flash_message = isForUpdate ? live_language.database_updated_success : `${live_language.database_created_success} : ${dbName}`;
					...
					>
						{isCreating ? (
							...
							>
								...
								>
									{isForUpdate ? live_language.update_db_infos_text : live_language.create_db_text}
								</motion.h2>
