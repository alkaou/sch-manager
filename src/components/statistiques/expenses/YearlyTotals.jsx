...
const YearlyTotals = ({ data, theme }) => {
  ...
  return (
    <motion.div 
      ...
      animate="visible"
    >
      <h3 className="text-xl font-bold mb-6 text-white text-center">{translate('expense_statistics_total', language)}</h3>
      <div className="overflow-y-auto scrollbar-custom pr-2">
        <div className="space-y-4">
          {data.map((year) => (
            <motion.div
              key={year.id}
              className={`flex items-center justify-between p-4 rounded-lg shadow-md ${theme === 'dark' ? 'bg-gray-700/50' : 'bg-gray-200'}`}
              variants={itemVariants}
            >
              <div className="flex items-center gap-4">
                <Calendar className="text-blue-400" size={24} />
                <span className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`}>{year.title}</span>
              </div>
