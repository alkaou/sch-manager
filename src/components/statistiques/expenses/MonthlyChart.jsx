...
const CustomTooltip = ({ active, payload, label, language, schoolYear }) => {
  if (active && payload && payload.length) {
    ...
    const monthName = date.toLocaleString(language === 'Anglais' ? 'en-US' : 'fr-FR', { month: 'long' });

    return (
      <div className="p-3 bg-gray-800 bg-opacity-90 text-white rounded-lg shadow-xl border border-gray-700">
        <p className="font-bold mb-1">{`${monthName} ${year}`}</p>
        <p className="text-sm">{`Total: ${new Intl.NumberFormat().format(payload[0].value)}`}</p>
      </div>
    );
  }
  return null;
...
const MonthlyChart = ({ data, theme, schoolYear }) => {
  ...
  const chartData = data.map(item => {
    ...
    const monthLabel = date.toLocaleString(language === 'Anglais' ? 'en-US' : 'fr-FR', { month: 'short' });
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold mb-4 text-white">{`${t('monthly_expenses_for')} ${schoolYear.title}`}</h3>
      <ResponsiveContainer width="100%" height="90%">
        <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#4A5568' : '#E2E8F0'} />
          <XAxis dataKey="monthLabel" stroke={theme === 'dark' ? '#A0AEC0' : '#4A5568'} fontSize={12} />
          ...
          <Tooltip content={<CustomTooltip language={language} schoolYear={schoolYear} />} cursor={{ fill: 'rgba(100, 116, 139, 0.1)' }} />
          <Legend formatter={(value) => <span className="text-white">{t(value)}</span>} />
          <Bar dataKey="total" name="total_expenses" fill="#3b82f6" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </motion.div>
  );
